...
The test suite is run via phpunit. Note that these are all live tests that must be run targeted at a real Infinity instance. Credentials can be provided by setting the environment variables in phpunit.xml; a sample is provided at phpunit.xml.dist.

To run the unit tests: `vendor/bin/phpunit --testsuite "Infinity API Unit Test Suites"`

To run the live tests: `vendor/bin/phpunit --testsuite "Infinity API Live Test Suites"`
 ...
 *
 * @var Infinity\API\Utilities\Auth
 */
```
...
When adding a resource, use traits to define available API calls. Resource traits are namespaced under `Infinity\API\Traits\Resource`.
...
Use `Infinity\API\Traits\Utility\InstantiatorTrait` when you want a resource to be chainable to other resources. See `Infinity/API/Resources/Servers.php`.

``` php
$this->client->servers()->sites()->findAll();
```
